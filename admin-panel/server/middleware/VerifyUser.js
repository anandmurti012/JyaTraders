...
const verifyUserToken = async (req, res,next) => {
    try {
        ...
        connection.query(`SELECT * FROM users WHERE email='${verify.email}'`, async function (error, result) {
            if (error) {
...
const verifyAdminToken = async (req, res, next) => {
    try {
        ...
        const verify = jwt.verify(token, SECRET_KEY);
        connection.query(`SELECT * FROM admins WHERE adminId='${verify.adminId}'`, async function (error, result) {
            if (error) {
                return res.status(404).json({ msg: error.sqlMessage });
            } else {
                const rootUser = result[0]
